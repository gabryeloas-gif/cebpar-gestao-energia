...
const specs: Record<string, { table: string; module: string; fields: string[]; required: string[] }> = {
  projects: { table: 'projects', module: 'projects', required: ['name','project_type','status','phase'], fields: ['code','name','short_name','related_project_id','organization','project_type','business_model','location','description','owner_name','team','companies','status','phase','neoenergia_quote_code','priority','criticality','dc_power_mwp','ac_power_mw','bess_power_mw','bess_energy_mwh','connection_voltage','generation_mode','distributor','annual_generation_mwh','customer_consumption_mwh','tariff_group','average_demand_kw','charger_power_kw','charger_unit_count','charging_time_hours','tariff_value_per_kwh','technical_notes','capex_estimated_cents','capex_contracted_cents','annual_opex_cents','annual_revenue_cents','sei_process','contract_number','contractor','contract_value_cents','start_date','planned_end_date','actual_end_date','next_milestone','next_milestone_due','physical_progress','schedule_status','delay_reason'] },
  crm: { table: 'crm_opportunities', module: 'crm', required: ['opportunity_name','company_name','stage'], fields: ['opportunity_name','company_name','contact_name','contact_role','contact_email','contact_phone','project_type','business_model','location','distributor','estimated_consumption_mwh','estimated_demand_kw','potential_power_mwp','stage','probability','expected_close_date','owner_name','next_action','next_action_due','project_id','notes','lost_reason'] },
...
for (const [route, spec] of Object.entries(specs)) {
  app.get(`/api/${route}`, async c => {
    ...
    if (q) { const searchField = route === 'projects' ? 'name' : route === 'activities' || route === 'documents' ? 'title' : route === 'crm' ? 'opportunity_name' : 'description'; clauses.push(`(CAST(t.id AS TEXT) LIKE ? OR COALESCE(t.${searchField},'') LIKE ?)`); values.push(`%${q}%`,`%${q}%`); }
